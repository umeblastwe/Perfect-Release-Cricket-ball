...
def process_bowling_video(video_path, output_path, job_id):
    ...
    try:
        ...
        # ── Rolling frame buffer so we can retrieve exact wrist-peak frame ──
        SNAP_BUFFER = 8        # keep last 8 frames (~320ms at 25fps)
        frame_buffer = []      # list of frame copies (most recent last)
        ...
        # Save release snapshot if captured
        ...
        summary = {
            ...
            "avg_arm_angle":     int(np.mean(arm_angles))        if arm_angles      else None,
...
def upload_video():
    ...
    # Clean up old analyzed files AND snapshots to save disk on free tier
...
def serve_static(filename):
    """Serve video (mp4) and snapshot (jpg) files."""
